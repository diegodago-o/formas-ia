...
import { getPendingVisits, deletePendingVisit, getDrafts, deleteDraft } from '../../services/localDB';
...
export default function MyVisits() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    await Promise.all([loadServer(), loadLocal(), loadDraftsLocal()]);
    setLoading(false);
  }, [loadServer, loadLocal, loadDraftsLocal]);
    ...
    <Layout title="Mis Visitas">
      ...
      {pending.length > 0 && (
        <div className={styles.syncBar}>
          <span>📥 {pending.length} visita{pending.length > 1 ? 's' : ''} por sincronizar</span>
          {online ? (
